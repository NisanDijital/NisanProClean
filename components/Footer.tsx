...
const Footer: React.FC = () => {
  ...
  const handleNewsletterSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    trackFormSubmit("footer_newsletter", { has_email: true });
    const subject = encodeURIComponent("NisanProClean kampanya bulteni");
    const body = encodeURIComponent(
      `Merhaba, ${cleanEmail} adresimle kampanya bultenine katilmak istiyorum.`
    );
    window.location.href = `mailto:nisanproclean@gmail.com?subject=${subject}&body=${body}`;
  };

  return (
    <footer className="bg-surface-dark border-t border-white/5 pt-20 pb-10" data-analytics-source="footer">
      <div className="max-w-7xl mx-auto px-4">
        <div className="grid md:grid-cols-4 gap-12 mb-16">
          <div className="col-span-1 md:col-span-1">
            ...
            <p className="text-gray-500 text-sm leading-relaxed mb-4">
              Afyonkarahisar merkezli ekip, yerinde koltuk ve doseme temizligini profesyonel ekipmanla uygular.
            </p>
            <a
            ...
            <div className="flex gap-3 mt-6">
              ...
              <span
                className="size-10 rounded-lg bg-white/5 border border-white/5 flex items-center justify-center text-gray-600 cursor-not-allowed"
                title="Facebook profili su an aktif degil"
              >
                <svg fill="currentColor" viewBox="0 0 24 24" className="size-5">
              ...
              <span
                className="size-10 rounded-lg bg-white/5 border border-white/5 flex items-center justify-center text-gray-600 cursor-not-allowed"
                title="X profili su an aktif degil"
              >
          ...
          <div>
            <h4 className="text-white font-bold mb-6">Bulten</h4>
            <p className="text-gray-500 text-sm mb-4">Kampanya ve duyurulari e-posta ile al.</p>
            <form className="flex gap-2" onSubmit={handleNewsletterSubmit}>
              ...
              <button
                type="submit"
                className="bg-primary hover:bg-cyan-600 text-white rounded-lg px-3 py-2 transition-colors"
                aria-label="Bultene katil"
              >
        ...
        <div className="border-t border-white/5 pt-8 flex flex-col md:flex-row justify-between items-center gap-4">
          <div className="flex flex-col md:flex-row items-center gap-4">
            <p className="text-gray-600 text-xs">(c) 2026 NisanProClean. Tum haklari saklidir.</p>
            <div className="flex items-center gap-2 px-3 py-1.5 bg-white/5 rounded-lg border border-white/10">
              <span className="material-symbols-outlined text-gray-400 text-sm">lock</span>
              <span className="text-xs text-gray-400 font-medium">Guvenli odeme altyapisi</span>
            </div>
          </div>
