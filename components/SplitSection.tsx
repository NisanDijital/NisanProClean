...
const SplitSection: React.FC<SplitSectionProps> = ({ 
  ...
  return (
    <div className="group grid lg:grid-cols-2 gap-12 items-center">
      <div className={`relative rounded-2xl overflow-hidden aspect-[4/3] border border-white/5 ${reverse ? 'lg:order-2 order-1' : 'lg:order-1 order-1'}`}>
        <div className={`absolute inset-0 ${overlayColor} group-hover:bg-transparent transition-all duration-500 z-10`}></div>
        <div 
      ...
      <div className={`flex flex-col gap-6 items-start ${reverse ? 'lg:order-1 order-2 lg:items-end lg:text-right' : 'lg:order-2 order-2'}`}>
        <div className={`${textColor} font-bold tracking-widest text-sm uppercase flex items-center gap-2 ${reverse ? 'lg:flex-row-reverse' : ''}`}>
          <span className={`w-8 h-[2px] ${lineColor}`}></span>
          {category}
        </div>
